...
- agent_spawn_guard: prompt_pattern_block_messageに理由行追加・override_instruction明確化 (issue_8139)

### SendMessage guard強化
- sendmessage_guardパターンのブラケット内文字数制限を10→5に変更 (issue_8134)
...
- `sendmessage_guard.block_message`をconfig.yaml管理化 — `[ステータス]`フォーマットを公式定義として導入 (issue_8139)
- `sendmessage_guard.exempt_types`をconfig.yaml公開 — shutdown_request等の免除タイプを設定可能化 (issue_8139)
...
- config.yamlチューニング検証テスト追加: SendMessageガード設定値検証(8件)・内容検証(3件)・agent_spawn_guard設定値検証(3件) (issue_8139)
...
Claude Code公式のhookイベントにはPreToolUse時点でcaller agentを識別するフィールドがない。従来のtranscript走査はファイル書込タイミングに依存し不安定だった。hookイベントのagent_idフィールド（v2.1.45で公式化）を直接参照する方式に全面移行し、判定の確実性と速度を大幅に改善した。
