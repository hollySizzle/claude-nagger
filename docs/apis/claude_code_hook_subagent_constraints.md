...
調査日: 2026/02/18 (v2.1.45), 2026/02/26 (v2.1.62), 2026/03/04 (agent_id方式正式採用), 2026/03/13 (v2.1.69 agent_id全hookイベント拡大確認)
...
| #15998 | CLOSED(重複) | agent_id/type追加提案。ボットにより自動クローズ。**v2.1.45で公式実装済み** |
...
- v2.1.45: agent_id/agent_typeがSubagentStart/Stopに公式追加
- v2.1.69: agent_idが全hookイベント（PreToolUse含む）に拡大提供
...
- 制約: transcript構造はundocumented（バージョン変更リスク）。なおagent_idはCC 2.1.45で公式化（2.1.69で全hookイベントに拡大）されたため、transcript走査による識別は不要
- 詳細: Redmine US #6928
...
- **層A**: agent_id方式で解決済み。CC 2.1.45でagent_id/agent_type公式化、2.1.69で全hookイベントに拡大。Anthropic公式対応(#16424)は完了
