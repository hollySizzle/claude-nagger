...
| #8590 | test_sendmessage_guard_hook.py | ステータス再設計（enum+宛先必須）、message/content参照修正、exempt_routes to修正 |
...
#### test_sendmessage_guard_hook.py（107テスト）
...
| M8 | apply_directions: leader_to_subagentのみ | config: `["leader_to_subagent"]` | leader→subagentのみ検証、subagent→leaderはスキップ ※#8618: 実環境で未動作。片方向フィルタのconfig読み込みパス問題 |
...
| M7a | 新ステータス送信許可 | `issue_XXXX [完了:leader]`等の新形式SendMessage | 許可（パターンマッチ） |
| M7b | 旧ステータス拒否 | `issue_XXXX [完了]`（宛先なし）SendMessage | ブロック（宛先必須） |
| M7c | messageフィールド参照 | 実ペイロードでSendMessage送信 | 許可（messageフィールド正常参照） |
| M7d | 構造化メッセージスキップ | shutdown_request等dictメッセージ | 許可（構造化メッセージapprove） |
| M7e | 複数行メッセージ先頭行検証 | 複数行メッセージのSendMessage | 先頭行のみ検証 |
...
| # | テスト | 操作 | 期待結果 |
|---|--------|------|----------|
| M14 | Bash禁止 | leaderがBash(非git)実行 | ブロック（gitコマンドのみ許可） |
| M15 | Grep禁止 | leaderがGrep実行 | ブロック（coder/researcherに委譲） |
| M16 | Edit/Write禁止 | leaderがEdit/Write実行 | ブロック（coderに委譲） |

### session_startup hook

| # | テスト | 操作 | 期待結果 |
|---|--------|------|----------|
| M17 | subagent規約通知 | coder/tester/tech-lead起動時 | 各ロール固有の規約がPreToolUseブロックで通知される |

### プラグインキャッシュ

| # | テスト | 操作 | 期待結果 |
|---|--------|------|----------|
| M18 | キャッシュ更新 | plugin.jsonバージョンバンプ→/reload-plugins | キャッシュ版config.yamlが最新に更新される |
