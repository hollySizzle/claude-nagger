...
3. PreToolUse `input_data`に`agent_id`なし → ~~Claude Code API制約~~（v2.1.69で解消: 全hookイベントにagent_id提供）
4. アトミック操作不在 → レースコンディションのリスク
5. ディレクトリリストが「クエリ」代替 → 横断的状態管理が困難

### Claude Code API制約（v2.1.69で大幅改善）

| イベント | agent_id | agent_transcript_path |
|---------|----------|----------------------|
| SubagentStart | あり（v2.1.45〜） | なし |
| PreToolUse | あり（v2.1.69〜） | なし |
| SubagentStop | あり（v2.1.45〜） | あり |

~~PreToolUseで「どのsubagentが呼んだか」を直接識別する手段がAPI側に存在しない。~~ → v2.1.69で全hookイベントにagent_id提供。Claimパターンは不要になった可能性あり（要検証）。
...
**Phase 2 - PreToolUse時（v2.1.69以降はagent_idあり）**:
- v2.1.69以降: agent_idで直接識別可能（Claim不要）
- v2.1.69未満フォールバック: 未処理subagentをFIFO Claimで割当（排他消去法を優先）
...
| SubagentStart公式化済み | CC 2.1.45でagent_id/agent_type公式化（2.1.69で全hookイベントに拡大）。破壊的変更リスクは低下 | SubagentStart依存ロジックを分離し、差し替え容易な設計は維持する |
